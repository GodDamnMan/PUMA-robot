...
# d = 10 cm future NOT FORGET!
q1, q2, q3, q4, q5, q6, q7 = symbols('q1 q2 q3 q4 q5 q6 q7')
dq1, dq2, dq3, dq4, dq5, dq6, dq7 = symbols('dq1 dq2 dq3 dq4 dq5 dq6 dq7')
ddq1, ddq2, ddq3, ddq4, ddq5, ddq6, ddq7 = symbols('ddq1 ddq2 ddq3 ddq4 ddq5 ddq6 ddq7')

theta = [q1, q2, q3, q4, q5, q6, q7]
dtheta = [dq1, dq2, dq3, dq4, dq5, dq6, dq7]
ddtheta = [ddq1, ddq2, ddq3, ddq4, ddq5, ddq6, ddq7]
...
M67 = (diff(diff(E_total, dq6), dq7))

M71 = (diff(diff(E_total, dq7), dq1))
M72 = (diff(diff(E_total, dq7), dq2))
M73 = (diff(diff(E_total, dq7), dq3))
M74 = (diff(diff(E_total, dq7), dq4))
M75 = (diff(diff(E_total, dq7), dq5))
M76 = (diff(diff(E_total, dq7), dq6))
M77 = (diff(diff(E_total, dq7), dq7))
...
def compute_coriolis_matrix(M, theta, dtheta):
    n = len(theta)
    C = Matrix.zeros(n, n)
    for k in range(n):
        for j in range(n):
...
dtheta_vec  = Matrix(dtheta)
ddtheta_vec = Matrix(ddtheta)

tau_sym = M*ddtheta_vec + C*dtheta_vec + G
tau_func = lambdify(theta + dtheta + ddtheta, tau_sym, 'numpy')

def compute_tau_numeric(q_vals, dq_vals, ddq_vals):
    return np.array(
        tau_func(
            *q_vals,
            *dq_vals,
